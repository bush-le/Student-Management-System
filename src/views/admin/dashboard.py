...
class AdminDashboard(ctk.CTkFrame):
    def __init__(self, parent, app, user):
        ...
        self.user_menu = None # Variable to track dropdown menu
        self.active_view = "dashboard" # Track current active view
    ...
    def create_sidebar(self):
        ...
        self._sidebar_btn(sidebar, 2, "Dashboard", key="Dashboard", command=self.refresh_home)
        ...
        self._sidebar_btn(sidebar, 8, "Announcements", key="Announcements", command=lambda: self.switch_view("Announcements", self.show_announcements)) # Announcements button
        # Footer
    ...
    # =========================================================================
    # 2. HEADER SECTION (UPDATED: USER MENU DROPDOWN)
    # =========================================================================
    def create_header(self):
        ...
        # Assign click event to child components to ensure click functionality
        for w in [lbl_name, lbl_role, avatar, lbl_avatar]:
            w.bind("<Button-1>", self.toggle_user_menu)

    # --- USER MENU HANDLING FUNCTIONS ---
    def toggle_user_menu(self, event=None):
        """Shows or hides the user menu."""
    ...
    def show_home(self):
        ...
        self._draw_action_btn(actions_grid, "Add New Student", "Create a new student profile", lambda: self.switch_view("Students", self.show_students)) # Button for adding new student
    ...
    def refresh_home(self):
        """Refreshes dashboard stats and shows home view"""
        self.show_home()
        run_in_background(
            self.controller.get_dashboard_stats,
            on_complete=self._update_stats,
    ...
    def clear_content(self):
        # Close menu if open when switching pages
        if self.user_menu and self.user_menu.winfo_exists():
            self.user_menu.destroy()
            self.user_menu = None
    ...
    def show_semesters(self):
        SemestersFrame(self.content_scroll, self.controller).pack(fill="both", expand=True)

    def show_students(self):
        StudentsFrame(self.content_scroll, self.controller).pack(fill="both", expand=True)

    def show_lecturers(self):
        LecturersFrame(self.content_scroll, self.controller).pack(fill="both", expand=True)

    def show_courses(self):
        CoursesFrame(self.content_scroll, self.controller).pack(fill="both", expand=True)

    def show_classes(self):
        ClassesFrame(self.content_scroll, self.controller).pack(fill="both", expand=True)

    def show_announcements(self):
        AnnouncementsFrame(self.content_scroll, self.controller, self.user.user_id).pack(fill="both", expand=True)