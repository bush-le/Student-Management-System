...
class ForgotPasswordView(ctk.CTkFrame):
    def __init__(self, parent, auth_controller, back_callback):
        ...
        # 3. STATUS LABEL (Để báo lỗi/thành công ngay dưới subtitle)
        ...
        self.send_btn = ctk.CTkButton(
            ...
            width=340, height=45, 
            fg_color="#367c74", hover_color="#2A6B63", # Màu xanh Teal đậm giống ảnh
            font=("Arial", 14, "bold"),
            command=self.send_code
        )
        self.send_btn.pack()

        # --- STEP 2: FORM OTP & PASSWORD (Mặc định ẩn) ---
        self.step2_frame = ctk.CTkFrame(self.inner, fg_color="transparent")
        ...
        ctk.CTkButton(
            ...
            text="Reset Password", 
            width=340, height=45, 
            fg_color="#E76F51", hover_color="#D65A3F", # Màu cam nổi bật cho hành động quan trọng
            font=("Arial", 14, "bold"),
            command=self.do_reset
        ).pack()

        # 4. FOOTER LINK (Back to Login)
        # Style giống ảnh: "Remember password? Sign in"
        ...
        link_btn = ctk.CTkButton(
            ...
            text_color="#367c74", # Màu xanh Teal chữ
            font=("Arial", 12, "bold"), 
            hover_color="white",
            width=50,
    ...
    def create_icon_header(self):
        """Tạo icon mũ cử nhân trong khung tròn/vuông bo góc"""
        # Container cho icon (Giả lập hình vuông bo góc màu xanh nhạt)
        icon_bg = ctk.CTkFrame(self.inner, fg_color="#E0F2F1", width=70, height=70, corner_radius=15)
        icon_bg.pack()
        icon_bg.pack_propagate(False) # Giữ kích thước cố định

        # Icon Emoji ở giữa
        ctk.CTkLabel(icon_bg, text="🎓", font=("Arial", 32)).place(relx=0.5, rely=0.5, anchor="center")
    ...
    def send_code(self):
        ...
        success, msg = self.controller.request_password_reset(email)
        
        self.send_btn.configure(state="normal", text="Send Recovery Code") # Reset nút
    ...
    def do_reset(self):
        ...
        success, msg = self.controller.reset_password(self.current_email, otp, new_pass)
        
        if success:
            messagebox.showinfo("Success", "Password reset successfully!\nPlease sign in with your new password.")
            self.back_callback() # Quay về trang login
        else:
            self.status_lbl.configure(text=msg, text_color="#DC2626")